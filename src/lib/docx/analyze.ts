...
/** Número seguido apenas de espaço (ex.: "05 Substantivo concreto ou abstrato?").
 * Só é aceito quando o restante tem cara inequívoca de enunciado/título de questão. */
const RE_QUESTAO_NUMERO_ESPACO = /^\s*(\d{1,3})\s+(?=\S)/;
const RE_INICIO_ENUNCIADO = /^(?:classifique|identifique|assinale|marque|responda|explique|complete|relacione|retire|substantivo|leia|observe|analise|calcule|determine)\b/i;
...
function detectarQuestao(texto: string): MarcaQuestao | null {
  ...
  // Alguns professores digitam "05 Substantivo..." sem ponto/parêntese.
  // Não aceitamos qualquer número + espaço para evitar transformar anos, valores
  // e itens internos de associação em questões. O restante precisa começar com
  // um comando/título típico de enunciado.
  const porEspaco = RE_QUESTAO_NUMERO_ESPACO.exec(limpo);
  if (porEspaco) {
    const numero = Number(porEspaco[1]);
    const resto = limpo.slice(porEspaco[0].length).trim();
    if (numero >= 1 && numero <= 300 && RE_INICIO_ENUNCIADO.test(resto)) {
      return { numero, rotulo: porEspaco[0], soMarcador: false, fraco: false };
    }
  }
...
type Cand = {
  ...
  parenteses: boolean;
  /** início com cara de comando/título de questão; ajuda a preservar questões
   * quando a prova original tem numeração repetida ou fora de sequência. */
  instrucao: boolean;
};
...
/**
 * Rejeita linhas internas de associação/listas que começam por número e por
 * acaso parecem uma questão. Ex.:
 * "1. Cardume ( ) ... 2. Enxame ( ) ... 3. Alcateia ...".
 * O bloco inteiro deve continuar dentro da questão "Relacione as colunas".
 */
function pareceListaInternaNumerada(texto: string, rotulo: string) {
  const resto = texto.trim().slice(rotulo.trim().length).trim();
  if (!resto) return false;
  if (RE_INICIO_ENUNCIADO.test(resto)) return false;

  const marcadoresDepois = resto.match(/\d{1,3}\s*[.)\-–—:]\s*/g) ?? [];
  const lacunasAssociacao = (resto.match(/\(\s*\)/g) ?? []).length;
  return marcadoresDepois.length >= 1 && lacunasAssociacao >= 2;
}
...
export async function analisarDocx(bytes: ArrayBuffer, nomeArquivo = "documento.docx"): Promise<AnaliseProva> {
  ...
  blocos.forEach((b, idx) => {
    ...
    if (q) {
      // Uma lista/coluna de associação pode começar com "1. ..." e conter
      // "2. ...", "3. ..." no MESMO bloco. Isso é conteúdo da questão, não
      // uma nova questão.
      if (pareceListaInternaNumerada(b.texto, q.rotulo)) return;
      ...
      candidatos.push({
        ...
        instrucao: /^(?:classifique|identifique|assinale|marque|responda|explique|complete|relacione|retire|substantivo|leia|observe|analise|calcule|determine)\b/i.test(
          b.texto.trim().slice(q.rotulo.trim().length).trim(),
        ),
      });
      return;
    }
  ...
  // A sequência crescente continua eliminando falsos positivos numéricos, mas
  // provas reais frequentemente têm numeração repetida/fora de ordem (ex.:
  // vários blocos chamados 01, 02, 05). Nesses casos, um cabeçalho com verbo
  // de comando é uma evidência forte de nova questão e não pode ficar grudado
  // na questão anterior — caso contrário seu número antigo reaparece após o
  // embaralhamento.
  const sequencia = melhorSequencia(candidatosFiltrados);
  const escolhidos = new Map<number, Cand>();
  sequencia.forEach((c) => escolhidos.set(c.idx, c));
  candidatosFiltrados
    .filter((c) => !c.auto && !c.parenteses && c.instrucao)
    .forEach((c) => escolhidos.set(c.idx, c));
  const inicios = Array.from(escolhidos.values()).sort((a, b) => a.idx - b.idx);
